...
if __name__ == "__main__":
    ...
    # 画像フォルダのパスを設定
    clear_image_folder = './data/all_data/test/clear'
    ...
    for k in tqdm(range(5)):
        image_folder = '/workdir/data/all_data/test/rain'
        clear_image_files = os.listdir(image_folder)
        ...
        for image_file in clear_image_files:
             if 'MSRA' in image_file:
                ...
                propose_mask, alpha, beta, gamma= propose_predict(image_file, prompt)
                ...
                masks_path = clear_image_path_to_mask_path(image_file)
                mask = np.load(masks_path)
                target = torch.tensor(mask).unsqueeze(1).to('cuda')
                ...
                overflow_score_and_IoU_list.append([image_file,IoU_score_propose_best, overflow_score_propose, alpha.item(), beta.item(), gamma.item()])

        csv_filename = f"rain_score_alpha_analyze_point{k}.csv"
        os.chdir('/workdir/alpha_analyze/rain')
        # CSVファイルとして保存
        if os.path.exists(csv_filename):
            ...
            df = pd.DataFrame(overflow_score_and_IoU_list, columns=['clear_image_path','IoU_score_propose','overflow_score_propose', 'α', 'β', 'γ'])
